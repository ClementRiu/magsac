...
bool MAGSAC<DatumType, ModelEstimator>::run(
    ...
    std::vector<size_t> inliersIdxsSaved;
    std::vector<double> weightsSaved;
    return run(points_, confidence_, estimator_, sampler_, obtained_model_, iteration_number_, model_score_,
               inliersIdxsSaved, weightsSaved);
}
...
bool MAGSAC<DatumType, ModelEstimator>::run(
    ...
    std::vector<size_t> inliersIdxsToSave;
    ...
    while (mininum_iteration_number > iteration ||
        ...
        for (const auto &model : models) {
            ...
            if (so_far_the_best_score < score) {
                ...
                max_iteration = MIN(max_iteration,
                                    last_iteration_number); // Update the max iteration number, but do not allow to increase
                inliersIdxsSaved = inliersIdxsToSave;
                weightsSaved = weightsToSave;
            }
        }
...
bool MAGSAC<DatumType, ModelEstimator>::sigmaConsensus(
    ...
    std::vector<size_t> inliersIdxsSaved;
    std::vector<double> weightsSaved;
    return sigmaConsensus(points_, model_, refined_model_, score_, estimator_, best_score_, inliersIdxsSaved,
                          weightsSaved);
}
...
bool MAGSAC<DatumType, ModelEstimator>::sigmaConsensusPlusPlus(
    ...
    return sigmaConsensusPlusPlus(points_, model_, refined_model_, score_, estimator_, best_score_, inliersIdxsSaved,
                                  weightsSaved);
}
